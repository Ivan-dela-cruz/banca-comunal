<div>
    <div class="intro-y block sm:flex items-center h-10 mt-4">
        <h2 class="text-lg font-medium truncate mr-5">Depositos</h2>
    ...
    <div class=" mt-5 intro-y box p-5">
        ...
        <div class="grid grid-cols-1 gap-4  md:grid-cols-2 mb-5">
            ...
            <div>
                <label>Monto</label>
                <input {{$action === 'POST' ? '' : 'disabled'}} wire:model="deposit" type="text" class="input w-full border mt-2" placeholder="00.00">
                @error('deposit') <span class="text-theme-6">{{$message}}</span> @enderror
            </div>
            <div>
    ...
    <div class="col-span-12 mt-6">
        <div class="intro-y block sm:flex items-center h-10">
            <h2 class="text-lg font-medium truncate mr-5">Detalle de Depositos  </h2>
            <div class="flex items-center sm:ml-auto mt-3 sm:mt-0">
               <p hidden> {{$type = "deposito"  }}</p>
            ...
            <table class="table table-report sm:mt-2">
                ...
                @foreach ($transactions as $data)
                    <tr class="intro-x">
                        ...
                        <td class="w-40 text-center">
                            <span class="px-2 py-1 rounded-full text-white {{$data->status === 'Finalizado' ? 'bg-theme-9':'bg-theme-6'}}">{{ $data->status }}</span>
                        </td>
